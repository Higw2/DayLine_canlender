...
随后在 Ubuntu 应用列表搜索 **DayLine**。如需登录后自动显示桌面日程：
...
- 显示桌面后，可以按住桌面卡片顶部的“时序”标题区域拖动；在 GNOME X11 下位置会自动保存，并在下次启动时恢复。
- 显示桌面后，桌面卡片顶部的拖拽区域显示实时时间，按住即可在桌面自由拖动；在 GNOME X11 下位置会自动保存，并在下次启动时恢复。
- 支持偏好设置：可自定义主题强调色（8 种预设风格与自由调色板）、桌面背景主题（暗夜沉静、清爽明亮、主题沉浸）与界面字体大小缩放。
